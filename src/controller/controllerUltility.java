...
public class controllerUltility {
    public ArrayList<BookReaderManagement> updateBRMInfo(
            ArrayList<BookReaderManagement> list, BookReaderManagement brm) {
        boolean isUpdate = false;
//        isupdate:đã được update hay chưa
...
//            list se them doi tuong vao cuoi
    ...
    public void deleteBookById(ArrayList<Book> list, int id) {
        int position = -1;
        for (int i = 0; i < list.size(); i++) {
    ...
    public void deleteReaderById(ArrayList<Reader> list, int id) {
        int position = -1;
        for (int i = 0; i < list.size(); i++) {
            if(list.get(i).getReaderID() == id){
                position = i;
            }
        }
        if(position != -1){
